...
Ben Schwartz: Asks if response code should be 200 or 300. Mike confirmed that the 200 is intentional code. Ben asks for consistency of Location header accross 200 and 300 response codes. Ben provides an example that redirects do not really work with DoH. He agrees with Martin's interpretation.
...
Open question (issue #3000) about singaling FIN vs RST. Ben explains the difference between FIN and RST. In HTTP/1.1 FIN vs RST is not specified, but in HTTP/2 this is signaled through Stream closure error. Ben discusses proposals to encode this in CONNECT-TCP. One option is a new Capsule type for graceful closure.
...
Ben thinks that we should set a deadline for the decision and make a consensus call. Tommy highlights that there were no strong opinions on this topic and proposes for Ben to make a decision to crystalize the consensus.
...
Lucas Pardue: this is a mainstream solution that does not break HTTP symantecs and suggests to keep going with this proposal.
...
Martin: key challange would be to work out stream lifecycle. But streams has all the right properties for it.
...
## [No-Vary-Search](https://datatracker.ietf.org/doc/draft-ietf-httpbis-no-vary-search/): Domenic Denicola, presenting remotely
Since last meeting:
...
## [Incremental HTTP Messages](https://datatracker.ietf.org/doc/draft-kazuho-httpbis-incremental-http/): Kazuho Oku, presenting in-person
...
Kazuho: Do you have preferences regarding signaling mechanisms?
MT: No other signaling preferences.
...
Kazuho: For hard fail, it's important for certain applications. It's true you might not know who the intermediary is, and in that case we can require. There might be some value in requiring specific behaviors for services that actually implement this.
...
Kazuho: I'm not sure if the perople asking for buffer mode actually wants that ahrd fail mode or just the preference.
...
## [HTTP Unencoded Digest](https://datatracker.ietf.org/doc/draft-pardue-httpbis-identity-digest/): Lucas Pardue, presenting remotely
...
## [Delete-Cookie](https://datatracker.ietf.org/doc/draft-deletecookie-weiss-http/) and [_HttpOnly Prefix](https://datatracker.ietf.org/doc/draft-httponlyprefix-weiss-http/): Yoav Weiss, presenting remotely

Tommy: Regarding process stuff, we are not going to update RFC6265bis, unless there's extremeley strong indications.
There's another effort to revise that yet again. We haven't done any adoption calls on that, I would advice if the authors and group participants could collecticelty think those should get folded together, that could essentially be part of one adoption effort.
...
## [Cookies: HTTP State Management Mechanism](https://datatracker.ietf.org/doc/draft-annevk-johannhof-httpbis-cookies/): Johann Hofmann, presenting remotely
...
Johann: You can probably do that. It's not adopted yet oddicially, so I'm not sure. You need a few bits about properly specifying the bit about paritioning cookie access and then get merged.
...
## [Critical CH](https://datatracker.ietf.org/doc/draft-victortan-httpbis-chr-critical-ch/) and [Accept-CH Frame](https://datatracker.ietf.org/doc/draft-victortan-httpbis-chr-accept-ch-frame/): Victor Tan
...
Victor: For the major version, we have said that's eentropy. Version can be used to identify when you combine it with all the other information. I think we have an explainer for that.
